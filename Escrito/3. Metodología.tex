...
El código completo de este trabajo (preprocesamiento, entrenamiento, evaluación y los notebooks de análisis) está disponible públicamente en el repositorio de GitHub \texttt{benjamitchell/}\allowbreak\texttt{Tesis-TFT-Electricidad}.\footnote{El entorno local usado para preprocesamiento, evaluación y análisis fija las siguientes versiones: Python 3.12, PyTorch 2.9, PyTorch Lightning 2.1, PyTorch Forecasting 1.5, Prophet 1.2, scikit-learn 1.8, pandas 2.3. El entrenamiento en GPU se realizó en un entorno de \textit{cluster} separado (\texttt{conda}, ROCm) cuyas versiones exactas no quedaron registradas, una limitación de reproducibilidad declarada explícitamente en el Capítulo 5.}
...
Los datos se dividen en tres conjuntos cronológicamente ordenados, respetando estrictamente la causalidad temporal: ningún conjunto contiene observaciones posteriores a las del conjunto siguiente. La división adoptada es 70\% entrenamiento, 15\% validación y 15\% prueba, que corresponde a los siguientes períodos:
...
\begin{table}[htbp]
    ...
    \begin{tabular}{llll}
        ...
        Prueba        & 2025-05-13 07:00 & 2026-04-24 00:00 & 15\% \\
        \bottomrule
    \end{tabular}
...
Como cierre aplicado, se evalúa si la ventaja de S+C-LN sobre la línea base de persistencia (Sección 4.1) se sostiene bajo una función de pérdida asimétrica, en vez de las métricas simétricas (MAE, RMSE) usadas en el resto de este trabajo. Se emplea la pérdida lineal-lineal (\textit{linlin} o \textit{pinball}) con parámetro $\tau \in (0,1)$:
\[
    L_\tau(e) = \tau \max(e,0) + (1-\tau)\max(-e,0)
\]
con $e = y_t - \hat{y}_t$, que penaliza la subestimación del precio ($e>0$) con peso $\tau$ y la sobreestimación ($e<0$) con peso $1-\tau$; $\tau=0.5$ recupera el caso simétrico. Se calcula $L_\tau$ para S+C-LN y para la persistencia (Sección 4.1) sobre el conjunto de prueba de las ocho barras, para $\tau \in \{0.2, 0.35, 0.5, 0.65, 0.8\}$, y se reporta el \textit{skill score}:
\[
    1 - \frac{L_\tau(\text{S+C-LN})}{L_\tau(\text{persistencia})}
\]
...
Para aislar cuánto aporta la información exógena (posición solar y clima) por sobre la autocorrelación de la propia serie, se entrena una variante "solo precios" del TFT (sin ningún \texttt{known\_real} ni variable climática, únicamente la serie de costo marginal estandarizada como \texttt{unknown\_real}) para cuatro barras (Atacama, Cardones, P.Montt, Tarapaca), usando el mismo período histórico y las mismas fechas de corte entrenamiento/validación/prueba que el pipeline principal (Tabla \ref{tab:division}), de modo que la única diferencia respecto de S+C sea la presencia o ausencia de las features exógenas.
...
Como seguimiento acotado a este resultado, se evalúa además si aumentar la capacidad del modelo (duplicando \texttt{hidden\_size}, de 32 a 64, y \texttt{hidden\_continuous\_size}, de 8 a 16, sin modificar ningún otro hiperparámetro) atenúa la degradación observada, para dos barras (Atacama y Puerto Montt, la de mejor y peor desempeño del pipeline principal, respectivamente), dado el costo computacional de este experimento adicional (entre 4 y 5 horas por barra en GPU para el modelo de mayor capacidad, medido directamente).
