...
def tft_model(dataset, barra, train_dl, val_dl, 
    ...
    # Creamos el modelo
    # ← LOSS: MAE para early stopping directo sobre la métrica objetivo
    #         Cambiar a QuantileLoss() si se quieren intervalos de predicción
...
def _listar_tfevents_ordenados(carpeta_base, max_profundidad=3):
    """Como buscar_tfevents, pero devuelve TODOS los archivos (no solo el más
    reciente), ordenados del más antiguo al más nuevo por su timestamp de
    inicio. Una corrida puede quedar repartida en varios archivos tras una
    caída de servidor + reanudación, o puede haber intentos previos
    abandonados con otros hiperparámetros — no se decide aquí cuál es
    "la" corrida vigente, eso lo hace quien grafica."""
        ...
        try:
            for item in os.listdir(carpeta):
                if item.startswith('_'):
                    continue  # p.ej. _pre_merge_backup: respaldos, no corridas activas
                ruta = os.path.join(carpeta, item)
                if os.path.isdir(ruta):
                    archivos_encontrados.extend(buscar_recursivo(ruta, profundidad + 1))
                elif item.startswith('events.out.tfevents'):
    ...
    archivos = buscar_recursivo(carpeta_base, 0)
    return sorted(archivos, key=extraer_timestamp)


def cargar_losses_tensorboard(carpeta_log, verbose=False):
    """Devuelve una lista de "corridas" encontradas en carpeta_log (una por
    archivo tfevents), cada una con su propio historial de train/val loss.

    Antes esta función tomaba solo el archivo más reciente por nombre, lo
    que falla si una corrida quedó repartida en varios archivos (p.ej. una
    caída de servidor + reentrenamiento donde la reanudación solo alcanzó a
    loguear la última época): el archivo "más nuevo" podía tener casi todos
    los datos vacíos mientras el historial real quedaba en un archivo más
    viejo. Ahora se devuelven todas las corridas por separado — sin
    fusionar — para que quien grafica decida cuál corresponde al modelo
    vigente en lugar de asumirlo.
    """
    ...
    eventos_paths = _listar_tfevents_ordenados(carpeta_log)
    if not eventos_paths:
        if verbose:
            print("No se encontraron archivos tfevents")
        return []

    corridas = []
    for eventos_path in eventos_paths:
        nombre = os.path.basename(eventos_path)
        try:
            ts_epoch = int(nombre.split('.')[3])
            etiqueta = datetime.fromtimestamp(ts_epoch).strftime('%Y-%m-%d %H:%M')
        except (IndexError, ValueError):
            ts_epoch = os.path.getmtime(eventos_path)
            etiqueta = datetime.fromtimestamp(ts_epoch).strftime('%Y-%m-%d %H:%M') + ' (mtime)'

        tamaño_mb = os.path.getsize(eventos_path) / (1024 * 1024)
        if verbose:
            print(f"    Leyendo: {nombre} ({tamaño_mb:.1f} MB) -> corrida {etiqueta}")
        ...
        try:
            ...
            if verbose:
                print(f"      Train: {len(train_loss)} épocas | Val: {len(val_loss)} épocas")
...
def grafico_losses(lista_barras, logs_dir, 
    ...
    for idx, barra in enumerate(lista_barras):
        ...
        # Cargar datos de Precios (una "corrida" por archivo tfevents encontrado)
        carpeta_precios = os.path.join(logs_dir, experimento_precios, barra)
        print(f"Precios:")
        corridas_p = cargar_losses_tensorboard(carpeta_precios, verbose=True)

        # Cargar datos de Residuos
        carpeta_residuos = os.path.join(logs_dir, experimento_residuos, barra)
        print(f"Residuos:")
        corridas_r = cargar_losses_tensorboard(carpeta_residuos, verbose=True)
        ...
        multi_p = len(corridas_p) > 1
        multi_r = len(corridas_r) > 1

        def _tonos(color_base, n):
            # La corrida mas reciente usa el color fuerte original; si hay
            # corridas mas viejas, se aclaran (mezcla con blanco) para
            # distinguirlas sin perder el color "fuerte" de siempre.
            from matplotlib.colors import to_rgb
            base = to_rgb(color_base)
            if n <= 1:
                return [base]
            return [tuple(1 - f + f * c for c in base)
                    for f in (0.35 + 0.65 * i / (n - 1) for i in range(n))]

        colores_train_p = _tonos('blue', len(corridas_p))
        colores_val_p = _tonos('orange', len(corridas_p))
        colores_train_r = _tonos('green', len(corridas_r))
        colores_val_r = _tonos('red', len(corridas_r))

        # Precios: una curva por corrida (si hay más de una, no se fusionan
        # ni se asume cuál es la vigente — se muestran todas etiquetadas
        # por fecha para que se decida visualmente)
        for i, corrida in enumerate(corridas_p):
            sufijo = f" ({corrida['etiqueta']})" if multi_p else ""
            if corrida['train_loss']:
                ax.plot(corrida['epochs_train'], corrida['train_loss'],
                        label=f"Train Precios{sufijo}", color=colores_train_p[i],
                        lw=2, linestyle='--', alpha=0.6)
            if corrida['val_loss']:
                ax.plot(corrida['epochs_val'], corrida['val_loss'],
                        label=f"Val Precios{sufijo}", color=colores_val_p[i], lw=2.5)
                best_val = min(corrida['val_loss'])
                best_epoch = corrida['epochs_val'][corrida['val_loss'].index(best_val)]
                ax.axvline(best_epoch, color=colores_val_p[i], linestyle=':', alpha=0.5, lw=1)

        # Residuos: idem
        for i, corrida in enumerate(corridas_r):
            sufijo = f" ({corrida['etiqueta']})" if multi_r else ""
            if corrida['train_loss']:
                ax.plot(corrida['epochs_train'], corrida['train_loss'],
                        label=f"Train Residuos{sufijo}", color=colores_train_r[i],
                        lw=1.8, linestyle='--', alpha=0.6)
            if corrida['val_loss']:
                ax.plot(corrida['epochs_val'], corrida['val_loss'],
                        label=f"Val Residuos{sufijo}", color=colores_val_r[i], lw=2)
                best_val = min(corrida['val_loss'])
                best_epoch = corrida['epochs_val'][corrida['val_loss'].index(best_val)]
                ax.axvline(best_epoch, color=colores_val_r[i], linestyle=':', alpha=0.5, lw=1)

        if multi_p or multi_r:
            ax.text(0.02, 0.02, 'mult. corridas detectadas — ver leyenda',
